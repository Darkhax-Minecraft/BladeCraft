...
public class BlockColoranator extends Block
{
	...
	private void setDefaultDirection(World world, int xCoord, int yCoord, int zCoord)
    {
        if (!world.isRemote)
        {
            ...
            if (Block.opaqueCubeLookup[block1ID] && !Block.opaqueCubeLookup[block2ID])
            {
            	dir = 3;
            }

            if (Block.opaqueCubeLookup[block2ID] && !Block.opaqueCubeLookup[block1ID])
            {
            	dir = 2;
            }

            if (Block.opaqueCubeLookup[block3ID] && !Block.opaqueCubeLookup[block4ID])
            {
            	dir = 5;
            }

            if (Block.opaqueCubeLookup[block4ID] && !Block.opaqueCubeLookup[block3ID])
            {
            	dir = 4;
            }
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer par5EntityPlayer, int par6, float par7, float par8, float par9)
    {
        ...
        else
        {
            ...
            {
                par5EntityPlayer.openGui(BladeCraft.instance, 0, world, x, y, z);
            }
    ...
    public static void updateBlockState(World par1World, int x, int y, int z)
    {
    ...
    public void breakBlock(World world, int x, int y, int z, int prevBlockID, int prevBlockMetadata)
    {
    	TileEntityColoranator tileentitycoloranator = (TileEntityColoranator)world.getBlockTileEntity(x, y, z);
